...
```text
@/Dockerfile 을 파워셀스크립트로 변환하려고 해, 20년 경력의 프로그래밍 전문가 관점에서 작업을 진행해줘.
- 코드는 최대한 간결하게 작성
- 작업 내용이 누락되면 안됨
- dockerfile에서 호출하는 외부 ps1 스크립트도 반영해야돼
- tool을 설치하고 바로 다음 라인에서 실행하는 경우에 path문제로 실행을 하지 못하는 경우가 없도록 조치 필요
- .NET 등과 같이 필요한 부분도 감안하여 처리
```
