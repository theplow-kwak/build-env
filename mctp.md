...
## 기본 전송 단위 (BTU): MCTP의 최소 공통 분모 크기는 64바이트입니다

64바이트는 **모든 MCTP 장치가 공통적으로 지원해야 하는 최소 크기**를 의미하며, 실제 전송 단위(Transmission Unit)는 이보다 **더 클 수 있습니다**.

소스에서 설명하는 구체적인 내용은 다음과 같습니다.

### 1. BTU(Baseline Transmission Unit)의 역할

* **최소 공통 분모:** BTU는 MCTP 네트워크 내에서 패킷이 문제없이 라우팅될 수 있도록 보장하는 **필수적인 최소 크기**입니다.
* **상호 운용성 보장:** 모든 MCTP 제어 메시지를 지원하는 엔드포인트는 BTU(64바이트) 이하의 패킷을 반드시 수용할 수 있어야 합니다. 또한, 모든 메시지 유형은 BTU보다 크지 않은 패킷으로도 전달될 수 있는 기능을 갖춰야 하는데, 이는 64바이트만 지원하는 브릿지가 경로에 있을 수 있기 때문입니다.

### 2. 더 큰 전송 단위 사용 가능성

* **확장 지원:** 엔드포인트는 64바이트보다 **더 큰 전송 단위를 지원할 수 있습니다**.
* **협의 및 협상:** 두 엔드포인트 간에 더 큰 전송 단위를 사용하기로 협의(Negotiation)가 된다면, 해당 크기에 맞춰 패킷을 주고받을 수 있습니다.
* **증가 단위:** 예를 들어, `Query Hop` 명령의 응답을 보면 전송 단위 크기를 BTU인 64바이트부터 시작하여 **16바이트 단위로 증가**시켜 보고하도록 되어 있습니다(예: 0x0000 = 64바이트, 0x0001 = 80바이트 등).

### 3. 패킷 구성 시의 규칙

* **최소 크기 유지:** 메시지를 여러 패킷으로 나누어 보낼 때, 마지막 패킷을 제외한 **모든 패킷의 페이로드 크기는 최소한 BTU(64바이트) 이상**이어야 합니다.
* **일관성:** 한 메시지에 속한 패킷들은 마지막 패킷을 제외하고 모두 동일한 크기를 유지해야 합니다.

요약하자면, **64바이트는 "반드시 지원해야 하는 바닥"**이며, 시스템의 성능이나 장치의 성능에 따라 엔드포인트끼리 서로 합의한다면 **그 이상의 크기로 패킷을 구성하여 전송 효율을 높이는 것이 가능**합니다.
